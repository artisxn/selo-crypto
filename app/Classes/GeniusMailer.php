...
class GeniusMailer
{
    ...
    public function sendInvoiceOrder(array $mailData, array $data)
    {
        $setup = Generalsetting::find(1);
        ...
        $objDemo->from = $setup->from_email;
        $objDemo->title = $setup->from_name;
        $objDemo->subject = $temp->email_subject.' #'.$data['invoice'];
        
        

        try{
            Mail::send(['html' => 'includes.invoiceEmail'],$data, function ($message) use ($objDemo) {
                $message->from($objDemo->from,$objDemo->title);
                $message->to($objDemo->to);
                $message->subject($objDemo->subject);
            });
    ...
    public function sendSuccessPayment(array $mailData, array $data)
    {
        $setup = Generalsetting::find(1);
        ...
        $objDemo->from = $setup->from_email;
        $objDemo->title = $setup->from_name;
        $objDemo->subject = 'PEMBAYARAN BERHASIL'.' #'.$data['invoice'];
        
        

        try{
            Mail::send(['html' => 'includes.successPayment'],$data, function ($message) use ($objDemo) {
                $message->from($objDemo->from,$objDemo->title);
                $message->to($objDemo->to);
                $message->subject($objDemo->subject);
            });
