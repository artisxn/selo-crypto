...
class Cart extends Model
{
    ...
    public function __construct($oldCart)
    {
        if ($oldCart) {
            ...
            $this->totalPriceEDC = $oldCart->totalPriceEDC;
        }
    }

// **************** ADD TO CART *******************

    public function add($item, $id, $size,$color, $keys, $values, $weight, $toko, $is_dropship) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->publish_price, 'price_edc' => $item->price,  'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        ...
        if(!empty($weight)){
            $storedItem['weight'] = $weight;    
        }
        if(!empty($toko)){
            $storedItem['toko_id'] = $toko['id'];    
        }  
        if(!empty($toko)){
            $storedItem['toko_name'] = $toko['shop_name'];    
        }  
        if(!empty($toko)){
            $storedItem['slug_toko'] = $toko['slug_shop_name'];    
        }
        if (isset($toko['vendor_slug_shop_name']) && !empty($toko['vendor_slug_shop_name'])) {
            $storedItem['vendor_slug_shop_name'] = $toko['vendor_slug_shop_name'];
        }
        if($is_dropship !== TRUE){
            $storedItem['is_dropship'] = false;    
        }else{
            $storedItem['is_dropship'] = true;    
        }
        ...
        $storedItem['price'] = ($item->publish_price <> '' ? $item->publish_price : $item->price) * $storedItem['qty'];
        $storedItem['price_edc'] = $item->price * $storedItem['qty'];
...
// **************** ADD TO CART MULTIPLE *******************
    ...
    public function addnum($item, $id, $qty, $size, $color, $size_qty, $size_price, $size_key, $keys, $values, $weight, $toko, $is_dropship) {
        $size_cost = 0;
        $storedItem = ['qty' => 0,'size_key' => 0, 'size_qty' =>  $item->size_qty,'size_price' => $item->size_price, 'size' => $item->size, 'color' => $item->color, 'stock' => $item->stock, 'price' => $item->publish_price, 'price_edc' => $item->price, 'item' => $item, 'license' => '', 'dp' => '0','keys' => $keys, 'values' => $values];
        if($item->type == 'Physical')
        {
...
// **************** ADDING QUANTITY *******************
    ...
    public function adding($item, $id, $size_qty, $size_price) {
        ...
        // dd($item->publish_price);
        $storedItem['price'] = ($item->publish_price + (int) $storedItem['size_price']) * $storedItem['qty'];
        $storedItem['price_edc'] = ($item->price + (int) $storedItem['size_price'] )* $storedItem['qty'];        
...
// **************** REDUCING QUANTITY *******************
    ...
    public function reducing($item, $id, $size_qty, $size_price) {
        ...
        $storedItem['price_edc'] = $item->price * $storedItem['qty'];
        $storedItem['price'] =  $storedItem['item']['publish_price'] * $storedItem['qty'];
