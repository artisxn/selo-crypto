...
class PaymentController extends Controller
{
    ...
    public function payreturn()
    {
        ...
        if (Session::has('tempcart')) {
            ...
            $invoice = Session::get('tempinvoice');
        } else {
            $tempcart = '';
        ...
        return view('front.success', compact('tempcart', 'order', 'invoice'));
    }
...
// Set TCP timeout to 30 seconds
    ...
    {
        if ($request->amount == '' && (int) str_replace(['Rp', ',', '.'], '', $request->amount) < 10000) {
            return 'Minimum Total Pembelian adalah 10.000';
        }

        $totalOrder = str_replace(['Rp', ',', '.'], '', $request->amount);

        try {
            $invoice = mt_rand(5, 1000) . time();
            ...
            $urlGV .= "product=" . urlencode('Pembelian Via EDCSTORE') . "&";
            $urlGV .= "custom=" . urlencode($invoice) . "&";
            ...
            // $request->session()->push(Auth::user()->email, collect([$invoice, $totalOrder]));
            $result = '';
            $result .= '<input type="hidden" name="gudang_voucher" value="'.base64_encode($invoice).'">';
            $result .= '<iframe src="' . $urlGV . '&bank=1" width="100%;" height="700"></iframe>';
        } catch (\Throwable $th) {
            $result = '<div style="text-align: center;">Gagal. Silahkan Cek Koneksi Internet Anda</div>';
        }
