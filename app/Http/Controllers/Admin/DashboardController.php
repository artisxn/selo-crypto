...
class DashboardController extends Controller
{
    ...
    public function index()
    {
        ...
        $vendorPayment = Order::where('payment_status','=','paid')->where('status', 'completed')->where('vendor_payment', '0')->get();
        $dropshipPayment = Order::where('payment_status','=','paid')->where('status', 'completed')->where('is_dropship', '1')->where('dropshipper_payment', '0')->get();
        $days = "";
        $sales = "";
        for($i = 0; $i < 30; $i++) {
        ...
        return view('admin.dashboard',compact('pending','activation_notify','processing','completed','products','users','blogs','days','sales','pproducts','rorders','poproducts','rusers','referrals','browsers', 'paid', 'vendorPayment', 'dropshipPayment'));
    }
