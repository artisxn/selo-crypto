...
class ProductController extends Controller
{
    ...
    {
         $datas = Product::where('product_type','=','normal')->where('status_verified','=','1')->orderBy('id','desc')->get();
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('price', function(Product $data) {
                                ...
                                $price = round($data->publish_price <> '' ? $data->publish_price : $data->price * $sign->value , 2);
    ...
    {
         $datas = Product::where('status_verified','=','0')->orderBy('id','desc')->get();

         //--- Integrating This Collection Into Datatables
         return Datatables::of($datas)
                            ->editColumn('name', function(Product $data) {
                                $name = mb_strlen(strip_tags($data->name),'utf-8') > 50 ? mb_substr(strip_tags($data->name),0,50,'utf-8').'...' : strip_tags($data->name);
                                $id = '<small>ID: <a href="'.route('admin-prod-verify', $data->slug).'" target="_blank">'.sprintf("%'.08d",$data->id).'</a></small>';
                            ...
                            ->editColumn('category', function(Product $data) {
                                ...
                                $cat .= $data->subcategory <> '' ? " > ".$data->subcategory->name : null;
                                $cat .= $data->childcategory <> '' ? $data->childcategory->name : null;
                            ...
                            ->addColumn('status', function(Product $data) {
                                ...
                                $verify = $data->status_verified == '1' ? 'selected' : '';
                                $decline = $data->status_verified == '2' ? 'selected' : '';
                                $pending = $data->status_verified == '0' ? 'selected' : '';
                                return '<div class="action-list"><select class="process select droplinks '.$class.'"><option value="" '.$pending.'>Need Verification</option><option data-val="1" value="'. route('admin-prod-pending-verify',['id1' => $data->id, 'id2' => 1]).'" '.$verify.'>Verify</option><<option data-val="2" value="'. route('admin-prod-pending-verify',['id1' => $data->id, 'id2' => 2]).'" '.$decline.'>Decline</option>/select></div>';
                            })
                            ->rawColumns(['name', 'status'])
                            ->toJson(); //--- Returning Json Data To Client Side
    }

    public function updateVerify($id1, $id2)
    {
        $data = Product::findOrFail($id1);
        $data->status_verified = $id2;
        $data->verified_date = date('Y-m-d');
        $data->verified_by = Auth::guard('admin')->user()->email;
        $data->update();
    }
    //*** GET Request
    ...
    public function store(Request $request)
    {
        ...
        if (!empty($request->category_id)) {
          ...
          $input['publish_price'] = $input['price'];
          $input['price'] = $input['price'] - ( $input['price'] * ($gs->rate_company / 100) );
          $input['publish_previous_price'] = $input['previous_price'];
          $input['previous_price'] = $input['previous_price'] - ($input['previous_price'] * ($gs->rate_company / 100) );
      ...
      // return $request;
        ...
        if (empty($request->tags))
         ...
         if (!empty($request->category_id)) {
           ...
           $input['publish_price'] = $input['price'];
           $input['price'] = $input['price'] - ( $input['price'] * ($category->rate / 100) );
           $input['publish_previous_price'] = $input['previous_price'];
           $input['previous_price'] = $input['previous_price'] - ($input['previous_price'] * ($category->rate / 100) );
...
// PRODUCT DELETE ENDS
    ...
    public function verify($slug)
    {
        ...
        return view('admin.product.verify', compact('productt', 'vendors'));
