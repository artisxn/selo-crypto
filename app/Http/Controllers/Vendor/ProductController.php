...
class ProductController extends Controller
{
    ...
    {
    	 $user = Auth::user();
         $datas = $user->products()->where('product_type','normal')->where('status_verified','=','1')->orderBy('id','desc')->get();
    ...
    public function catalogdatatables()
    {
         ...
         return Datatables::of($datas)
                            ...
                            ->editColumn('price', function(Product $data) {
                                $sign = Currency::where('is_default','=',1)->first();
                                $price = $sign->sign.$data->publish_price <> '' ? $data->publish_price : $data->price;
                                return  $price;
                            })
                            ->addColumn('action', function(Product $data) {
                                $user = Auth::user();
    ...
    {
         $datas = Product::where('status_verified','=','0')->where('user_id', Auth::user()->id)->orderBy('id','desc')->get();
    ...
    {
        $user = Auth::user();
        $highlight = DB::table('highlight_product')->where('user_id', $user->id)->first();

        return view('vendor.product.index', compact('highlight'));
    }
    ...
    {
        return view('vendor.product.pending');
    }
    ...
    public function store(Request $request)
    {
        ...
        //--- Logic Section
            ...
            // Check Highlight Product Limit
            // if ($highlight->sisa_highlight_product < 1) {
            //     $input['is_highlight'] = '0';
            // }
        ...
        if (!empty($request->meta_tag))
         {
           ...
           if (!empty($request->category_id)) {
             ...
             $input['publish_previous_price'] = 0;
             $input['previous_price'] = 0;
        ...
        $notification->product_id = $lastid;
        $notification->pending_product = '1';
        $notification->save();
    ...
    public function update(Request $request, $id)
    {
        ...
        if (!$request->has('is_highlight') || $highlight->sisa_highlight_product < 1) {
            $input['is_highlight'] = '0';
        }
            //Check Types
