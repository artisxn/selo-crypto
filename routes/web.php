...
Route::group(['domain' => '{subdomain}.{domain}'], function() {
  ...
  if ($domain !== 'ecommerceapp01a.southeastasia.cloudapp.azure.com' && $domain !== 'edcstore.co.id') {
    $user = User::where('domain_name', $domain)->where('is_vendor', 2)->first();
    

    if ($user && $user->subscribes->status === 1 && $user->subscribes->is_dropship === '1') { 
      config(['dropship.is_dropship' => true]);
      Route::get('/', 'User\StoreController@vendorsubs')->name('user.vendor');
      Route::get('/item/{slug}','User\StoreController@product')->name('user.product');
      Route::get('/store','User\StoreController@vendorsubs')->name('front.vendor');
    }
...
//Route::group(['domain' => '{subdomain}.edc.com'], function() {
  //if($_SERVER['HTTP_HOST'] !== "edc.com"){
    //dd($_SERVER['HTTP_HOST']);
    // Route::get('/', 'User\StoreController@index')->name('user.vendor');
    // Route::get('/item/{slug}','User\StoreController@product')->name('user.product');
    // Route::get('/store','Front\VendorController@index')->name('front.vendor');
//  }
  // Route::get('/', 'User\StoreController@index')->name('user.vendor');
  // Route::get('/item/{slug}','User\StoreController@product')->name('user.product');
  // Route::get('/store','Front\VendorController@index')->name('front.vendor');
...
// ************************************ WEB VENDOR END SECTION **********************************************


// ************************************ ADMIN SECTION **********************************************

Route::prefix('Admin-317c')->group(function() {
  ...
  Route::get('/status_order/notf/show', 'Admin\NotificationController@trans_notf_show')->name('trans-order-notf-show');
  Route::get('/status_order/notf/count','Admin\NotificationController@trans_notf_count')->name('trans-order-notf-count');
  Route::get('/status_order/notf/clear','Admin\NotificationController@trans_notf_clear')->name('trans-order-notf-clear');


  Route::post('/notification/send','Admin\NotificationController@send_notif')->name('send-notif');
  Route::post('/notification/ds','Admin\NotificationController@notif_ds')->name('send-notif-ds');
  ...
  Route::get('/orders/vendor-payment', 'Admin\OrderController@vendorPayment')->name('admin-order-vendor-payment');
  Route::get('/orders/dropshipper-payment', 'Admin\OrderController@dropshipPayment')->name('admin-order-dropshipper-payment');
  ...
  Route::get('/products/pending/status/{id1}/{id2}', 'Admin\ProductController@updateVerify')->name('admin-prod-pending-verify');
  ...
  // VERIFY PRODUCT
  Route::get('/products/verify/{slug}', 'Admin\ProductController@verify')->name('admin-prod-verify');
// VERIFY PRODUCT END
  ...
  Route::get('/vendorsub/verif', 'Admin\VendorController@verifSubs')->name('admin-vendor-subs-verification');
  Route::get('/vendor/sub/verify/{id}/{status}', 'Admin\VendorController@verifySubs')->name('admin-vendor-subs-verify');
  Route::get('/vendors/subs/datatables/{status?}', 'Admin\VendorController@subsdatatables')->name('admin-vendor-subs-datatables');
  ...
  Route::get('/vendors/sub/edit/{id}', 'Admin\VendorController@editSubs')->name('admin-vendor-subs-edit');
...
// Verification Section Ends
 ...
 //------------ ADMIN PRODUCT DISCUSSION SECTION ENDS ------------
  ...
  Route::get('/disputes/status/{id}', 'Admin\MessageController@status')->name('admin-update-dispute');
...
// Payment Informations
  ...
  Route::get('/general-settings/gv/{status}', 'Admin\GeneralSettingController@gv')->name('admin-gs-gv');
// Payment Gateways
...
// Currency Settings
  ...
  Route::get('/subscribers/csv', 'Admin\SubscriberController@subs_csv')->name('admin-subs-download-csv');
  Route::get('/subscribers/excel', 'Admin\SubscriberController@subs_excel')->name('admin-subs-download-excel');

  });


  Route::group(['middleware' => ['permissions:report']], function () {
    Route::get('reports-order','Admin\ReportOrderController@index')->name('admin-report-order');
    Route::get('reports-payment','Admin\ReportOrderController@payment')->name('admin-report-payment');
    Route::post('reports-order','Admin\ReportOrderController@excel')->name('download-report-order');      
  });
...
//user payment edc cash
  Route::get('/edc-cash', 'User\EdcController@edccash')->name('user-edccash');  
  Route::post('/edc-cash/post-number', 'User\EdcController@post_number')->name('user-post-number'); 
  Route::post('/edc-cash/verifikasi', 'User\EdcController@verifikasi')->name('user-verikasi-number');  
//user payment ends cash
//user payment invoice
  Route::get('/invoice', 'User\InvoiceController@index')->name('user-invoice');
  Route::get('/invoice/{id}', 'User\InvoiceController@detail')->name('user-invoice-detail');  
//user payment ends cash
...
// User Orders
  ...
  Route::get('/order/confirm/{slug}', 'User\OrderController@confirmOrder')->name('user-confirm-order');

  Route::post('/order/track', 'User\OrderController@tracking')->name('user-order-tracking');

  Route::get('/address', 'User\AdressController@index')->name('user-address-index');
  Route::get('/address/add', 'User\AdressController@create')->name('user-address-add');
  Route::post('/address', 'User\AdressController@store')->name('user-address-store');
  Route::get('/address/remove/{id}', 'User\AdressController@destroy')->name('user-address-remove');
  Route::get('/address/edit/{id}', 'User\AdressController@edit')->name('user-address-edit');
  Route::post('/address/update/{id}', 'User\AdressController@update')->name('user-address-update');
...
// User Subscription
  ...
  Route::post('/molly/submit', 'User\MollyController@store')->name('user.molly.submit');
    ...
    // GudangVoucher Routes Subs
    Route::post('/gv/submit', 'User\GvController@store')->name('user.gv.submit');
    Route::get('/gv/{tn}', 'User\GvController@paySuccess')->name('user.gv.success');
...
// User Vendor Send Message
  ...
  Route::get('/user/count_message', 'User\MessageController@count_message');
...
// User Favorite Seller Ends
  ...
  // Notifications
  Route::get('/notifikasi/count','User\NotificationController@notifCount')->name('user-notif-count');
  Route::get('/notifikasi', 'User\NotificationController@index')->name('user-notif-index');
  Route::get('/notifikasi/clear', 'User\NotificationController@clearNotif')->name('user-notif-clear');
...
Route::prefix('vendor')->group(function() {
  ...
  Route::get('/store-edit', 'Vendor\VendorController@store_edit')->name('vendor-store-edit');
  Route::get('/vendor-profile', 'Vendor\VendorController@vendorprofile')->name('vendor-profile-edit');
  Route::post('/vendor-profile-update', 'Vendor\VendorController@storevendorprofile')->name('vendor-profile-store');


  Route::post('/store-update', 'Vendor\VendorController@store_update')->name('vendor-store-update');
    ...
    // dropship
    Route::get('/products/dropship/index', 'Vendor\DropshipController@indexDropship')->name('vendor-dropship-index');
    Route::get('/products/dropship/create', 'Vendor\DropshipController@createDropship')->name('vendor-dropship-create');
    Route::get('/products/dropship/api', 'Vendor\DropshipController@showProduct')->name('vendor-dropship-show-product');
    Route::get('dropship/{item}', 'Vendor\DropshipController@storeDropship')->name('vendor-dropship-store');
    Route::get('/products/dropship/datatables', 'Vendor\DropshipController@datatables')->name('vendor-dropship-datatables'); //JSON REQUEST
    Route::get('/products/dropship/apidrop', 'Vendor\DropshipController@productDropship')->name('vendor-dropship-apidrop'); //JSON REQUEST
    Route::get('/products/dropship/delete/{id}', 'Vendor\DropshipController@destroyDropship')->name('vendor-dropship-delete');
    // akhir dropship
  ...
  Route::post('/order/{id}/update-resi', 'Vendor\OrderController@updateresi')->name('vendor-order-resi');
  //------------ ADMIN CATEGORY SECTION ENDS------------
  ...
  Route::get('/products/pending', 'Vendor\ProductController@pending')->name('vendor-prod-pending');
  Route::get('/products/pending/datatables', 'Vendor\ProductController@pendingdatatables')->name('vendor-prod-pending-datatables');
...
Route::get('/package/delete/{id}', 'Vendor\PackageController@destroy')->name('vendor-package-delete');
  ...
  // Komplain Notification
  Route::get('/conv/notf/show', 'Vendor\NotificationController@conv_notf_show')->name('vendor-conv-notf-show');
  Route::get('/conv/notf/count','Vendor\NotificationController@conv_notf_count')->name('vendor-conv-notf-count');
  Route::get('/conv/notf/clear','Vendor\NotificationController@conv_notf_clear')->name('vendor-conv-notf-clear');
  // Komplain Notification Ends
  ...
  Route::get('/payment', 'Vendor\NotificationController@vendorPayment')->name('vendor-payment-index');
 ...
 Route::get('/disputes', 'Vendor\MessageController@adminDiscordmessages')->name('vendor-dmessage-index');
 Route::get('/disputes/datatables', 'Vendor\MessageController@datatables')->name('vendor-dmessage-datatables');
 Route::get('/disputes/details/{id}', 'Vendor\MessageController@dispute')->name('vendor-dmessage-details');
 Route::post('/disputes/post', 'Vendor\MessageController@postmessage')->name('vendor-message-store');
 Route::get('/disputes/load/{id}', 'Vendor\MessageController@messageshow')->name('vendor-message-load');
...
// ************************************ FRONT SECTION **********************************************
  ...
  if( config('dropship.is_dropship') !== FALSE ){
    Route::get('/', 'User\StoreController@vendorsubs')->name('front.index');
    Route::get('/store','User\StoreController@vendorsubs')->name('front.vendor');
  }else{
  ...
  Route::get('/city','Front\FrontendController@city')->name('front.city');
  Route::get('/district','Front\FrontendController@district')->name('front.district');
  Route::post('/zip','Front\FrontendController@zip')->name('front.zip');
  ...
  Route::get('/allcategory','Front\FrontendController@allcategory')->name('allcategory');

  //Invoice
  Route::get('/invoice', 'Front\InvoiceController@index')->name('invoice-index');
  //Invoice
  ...
  Route::get('/autosearch/product/{slug}/{user_id}','Front\FrontendController@autosearchbyuser');
  ...
  Route::get('/addcart/{id}/{slug?}','Front\CartController@addcart')->name('product.cart.add');
  Route::get('/addtocart/{id}/{slug?}','Front\CartController@addtocart')->name('product.cart.quickadd');
  ...
  // PAYMENT
  Route::post('/gv/payment', 'Front\PaymentController@gv')->name('gv-payment');
  ...
  Route::post('/checkout/check-kurir','Front\CheckoutController@checkkurir')->name('front.checkkurir');
  Route::get('/checkout/test-alamat','Front\CheckoutController@test_alamat');
  // NEW ROUTE FOR CHECK OUT
  ...
  Route::get('/testt', function () {
    ...
    $mailer = new GeniusMailer();
                ...
                $urlGV .= "amount=".urlencode(275000)."&";
                $urlGV .= "product=".urlencode(implode(',', ['baju']))."&";
                $urlGV .= "custom=".urlencode( 3081587112401 )."&";
                $urlGV .= "email=".urlencode('sultan@majapahit.id');
                $urlGV .= "&bank=1";
                ...
                $start = stripos($response, "<main");
                $end = stripos($response, "</main");
                ...
                // return $body;
    return $mailer->sendInvoiceOrder($data, ['invoice' => 3081587112401, 'method' => 'Bank Transfer', 'gv' => $urlGV, 'response' => $body, 'name' => Auth::user()->name]);
})->name('testt');
  ...
  Route::get('/order/check', 'Front\FrontendController@autoConfirmOrder')->name('order-check-confirm');
  Route::get('/order/check/cancel', 'Front\FrontendController@autoCancelOrder')->name('order-check-cancel');
  // CRONJOB ENDS
