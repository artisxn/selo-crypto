        <div class="col-lg-3 col-md-6">
          <div class="left-area">
            ...
            <div class="header-area" style="text-align: left !important; padding-left: 20px; border-radius: 10px 10px 0px 0px;">
              <h4 class="title" style="color: #A5A5A5 !important">
                {{ __("Kategori") }}
              </h4>
            </div>
            
            <div class="body-area" style="font-weight: 300; color: #212121;">
              <form id="catalogForm" action="{{ route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')]) }}" method="GET">
                ...
                <ul class="filter-list">
                  ...
                  <li>
                    <div class="content">
                        <a href="{{route('front.category', $element->slug)}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="category-link"> <i class="fas fa-plus"></i> {{$element->name}}</a>
                        @if(!empty($cat) && $cat->id == $element->id && !empty($cat->subs))
                            @foreach ($cat->subs as $key => $subelement)
                            <div class="sub-content open">
                              <a href="{{route('front.category', [$cat->slug, $subelement->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas {{ !empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs) ? 'fa-plus' : 'fa-minus' }}"></i>{{$subelement->name}}</a>
                              @if(!empty($subcat) && $subcat->id == $subelement->id && !empty($subcat->childs))
                                ...
                                <div class="child-content open">
                                  <a href="{{route('front.category', [$cat->slug, $subcat->slug, $childcat->slug])}}{{!empty(request()->input('search')) ? '?search='.request()->input('search') : ''}}" class="subcategory-link"><i class="fas fa-minus"></i> {{$childcat->name}}</a>
                                </div>
                                @endforeach
                              @endif
                            </div>
            ...
            <div class="header-area" style="text-align: left !important; padding-left: 20px;">
              <h4 class="title" style="color: #A5A5A5 !important">
                {{ __("Harga") }}
              </h4>
              {{-- <i style="" class="fas fa-chevron-circle-right"></i> --}}
            </div>

            <div class="body-area">
              <input type="number" min=0  name="min"  id="min_price" class="form-control" style="border-radius : 10px;" placeholder="Rp. Min"/>
              <br>
              <input type="number" min=0  name="max"  id="max_price" class="form-control" style="border-radius : 10px;" placeholder="Rp. Max"/>
            </div>
            ...
            <div class="body-area">
              @foreach ($vendorsLocation as $location)
              <input type="checkbox" id="{{ $location->name }}" name="city" value="{{ $location->city_id }}">
            ...
            <div class="body-area">
              @for ($i = 1; $i < 6; $i++)
                <input type="checkbox" id="{{ $i }}" name="rating" value="{{ $i }}">
                <label for="{{ $i }}">{{ $i }} {{ $i !== 5 ? 'Keatas' : null }}</label>
              @endfor
            </div>
            ...
            <div class="body-area">
                ...
                <label for="condition1">Baru</label>

                <input type="checkbox" id="condition2" name="condition" value="2">
                <label for="condition2">Second</label>
            </div> --}}
