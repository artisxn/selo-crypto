...
@section('content')
@php
$konfersi_edc = DB::table('generalsettings')
->where('id', '=', 1)
->first();
@endphp
		...
		<div class="row">
			<div class="col-lg-12">
				<ul class="pages">
					...
					<li class="">
						<a class=" {{ $tab_active }}" id="pills-step4-tab" data-toggle="pill"
							href="#pills-step4" role="tab" aria-controls="pills-step4" aria-selected="false">
							<span>4</span>{{ __('Selesai') }}
...
<section class="checkout no-garis">
	<div class="container">
		<div class="row">
			...
			{{-- @if (  Request::get('id') <> '' &&  base64_decode(Request::get('id')) <> '' )
				<div class="tab-pane fade show tempcart active" id="pills-step4" role="tabpanel" aria-labelledby="pills-step4-tab">
					    @if(!empty($tempcart))
			...
			{{-- @else --}}
				<div class="col-lg-8">
					...
					<form id="form-checkout" action="" method="POST" class="checkoutform">
									...
									<div class="content-box">
										...
										<div class="content">
											...
											<div class="ship-diff-addres-area d-none">
												...
												<div class="row">
													<div class="col-lg-12">
														<button type="button" class="mybtn1 mybtn1-first btn-alamat" style="float:right!important;">{{ __('Simpan Alamat') }}</button>							
													</div>
												</div>
											</div>
										...
										<div class="content" style="padding-top:20px!important;">
											...
											<div class="order-area">	
												...
												<label class="form-check-label" for="checkbox{{$i}}">
													Klik untuk checkout barang dari toko {{$i}}?
												</label>
				...
				<div class="col-lg-4">
					...
					<div class="right-area">
						<div class="order-box" style="background-color:#F6F6F6!important;padding-top:0px!important;">
							<h4 class="title" style="padding:5px 25px 5px!important;font-size:13px!important;color:#fff!important;background-color:#A5A5A5!important;margin-left:-26px!important;margin-right:-26px!important;">
								Detail Harga
							</h4>
						...
						<div class="total-price price-tax">
								...
								<p style="color:#212121!important;">
									<span id="pajaknya">
											{{ Session::has('cart') ? App\Models\Product::convertPrice(Session::get('cart')->totalPrice * ($gs->tax / 100) )  : '0.00' }}
									</span>
						...
						@endif
							...
							<!-- ongkir_edc -->
								...
								@if ($tab_active === 'disabled')
									...
									@endif -->								
									<span id="total-tagihan" style="color:#212121!important;">{{ App\Models\Product::convertPrice($totalPrice) }}</span>
								@else
...
<div class="modal fade" id="confirm-payment-edc" role="dialog">
	<div class="modal-dialog">
		<div class="modal-content">
			{{-- <div class="submit-loader">
					<img src="{{asset('assets/images/'.$gs->admin_loader)}}" alt="">
...
$('.packing').on('click',function(){
	...
	function reset_harga(){
		...
		$('#ongkir-edc').html(edc_ongkir + " EDCCASH");
	...
	$('.checkbox-toko').on('change', function() {
		...
		$( ".klik" ).click(function() {
			...
			if (payType=='Bank Transfer') {
				...
				$('#total-cost').html(edc_format+" EDCCASH").css({"color":"#212121"});
				var edc_format = convert_edc(ongkir_total);
				$('#total-ongkir').html(edc_format+" EDCCASH").css({"color":"#212121"});
				var edc_format = convert_edc(harga_total + ongkir_total+pajaknya);
				$('#total-tagihan').html(edc_format+" EDCCASH").css({"color":"#EF4623"});

				var format_pajak = convert_edc(pajaknya);
				$('#pajaknya').html(format_pajak+" EDCCASH");
		...
		var edc_format = convert_edc(harga_total_edc);
		$('#total-edc').html(edc_format+" EDCCASH");
		var ongkir_edc = convert_edc(ongkir_total);
		$('#ongkir-edc').html(ongkir_edc+" EDCCASH");

		var total_semua_edc = edc_format+ongkir_edc+pajaknya;
		$('#total-tagihan-edc').html(total_semua_edc.toFixed(3)+" EDCCASH");

		// console.log(pajaknya);
	...
	function convert_edc(price){
		var kon ={{$konfersi_edc->edccash_currency}};
		var tot = price /kon;
...
<script type="text/javascript">
	...
	// $('.mybtn1-first').on('click', function () {
	// 	var length_checkbox = $('.checkbox-toko');
	...
	// 	for (var i=0; i<length_checkbox.length; i++) {       
	// 	nama_toko = $(length_checkbox[i]).attr('toko');
	// 		$(length_checkbox[i]).prop( "checked", true );
    //     }		
	// });
	...
	$('#step3-btn').on('click',function(){
		...
		for (var i=0; i<length_checkbox.length; i++) {       
           if (length_checkbox[i].type == "checkbox" && length_checkbox[i].checked == true){
			  ...
			  review_produk[i] = $('.area-'+nama_toko).html();
           }else if(length_checkbox[i].checked == false){
			  review_produk[i] = '';
		   }
