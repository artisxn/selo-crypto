   ...
   <div class="container">
      <div class="row">
         <div class="col-lg-12">
            <ul class="pages">
               <li>
                  <a href="{{route('front.index')}}" style="color: #fff;">{{ $langg->lang17 }}</a>
               </li>
               @if (!empty($cat))
               <li>
               <a href="{{route('front.category', $cat->slug)}}" style="color: {{ !isset($subcat) || $subcat == '' ? '#EF4623' : '#fff' }};">{{ $cat->name }}</a>
               </li>
               @endif
               @if (!empty($subcat))
               <li>
                  <a href="{{route('front.category', [$cat->slug, $subcat->slug])}}" style="color: {{ !isset($childcat) || $childcat == '' ? '#EF4623' : '#fff' }};">{{ $subcat->name }}</a>
               </li>
               @endif
               @if (!empty($childcat))
               <li>
                  <a href="{{route('front.category', [$cat->slug, $subcat->slug, $childcat->slug])}}" style="color: #EF4623">{{ $childcat->name }}</a>
               </li>
               @endif
...
@if(!empty($cat) && $cat->banner != null)
<div class="container">
  <div class="jumbotron jumbotron-fluid" style="padding:0;background-color:#ffff;">
    <div class="row">
      <div class="col-md-12">
        <img src="{{asset('assets/images/categories/'.$cat->banner) }}" alt="" class="img-fluid" style="border-top-left-radius:0px;border-top-right-radius:0px; border-bottom-left-radius:10px;border-bottom-right-radius:10px;width:100%;height:272px;margin-bottom:-70px;">
      </div>
    </div>
  </div>
</div>
...
<script>
  ...
  function filter() {
    ...
    let  city = [];
    $("input[name='city']:checked").each(function (index, element) {
      city[index]       = element.value;
    });

    if (city.length >= 1) {
      if (filterlink == '') {
        filterlink += '{{route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')])}}' + '?'+'city='+city;
      } else {
        filterlink += '&'+'city='+city;
      }
    }

    let  rating = [];
    $("input[name='rating']:checked").each(function (index, element) {
      rating[index]       = element.value;
    });

    if (rating.length >= 1) {
      if (filterlink == '') {
        filterlink += '{{route('front.category', [Request::route('category'), Request::route('subcategory'), Request::route('childcategory')])}}' + '?'+'rating='+rating;
      } else {
        filterlink += '&'+'rating='+rating;
      }
    }
