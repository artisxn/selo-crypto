...
foreach($cart->items as $product){
    $item = App\Models\Product::where('id','=', $product['item']['id'])->firstOrFail();

                                    if ($product['is_dropship'] === true) {
                                        $feeDropshipper += feeDropshipper($product['price']);                                     
                                        $feeCompany += feeCompany($product['price']) - @feeDropshipper($product['price']);
                                    }else{
                                        $feeCompany += feeCompany($product['price']);
                                    }

                                    $feeSeller += feeVendor($product['price']);
}

// $feeSeller = $order->pay_amount - $feeCompany;

// $feeCompany = $feeCompany - $feeDropshipper;
@endphp
        ...
        <div class="row">
                    ...
                    <div class="table-responsive-sm">
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                <th width="45%">{{ __('Fee Company') }}</th>
                                <th width="10%">:</th>
                                <td width="45%">{{ $order->currency_sign }} @rp($feeCompany)</td>
                                </tr>
            ...
            <div class="col-lg-6">
                <div class="special-box">
                    <div class="heading-area">
                        <h4 class="title">
                            {{ $order->is_dropship === '1' ? __('Dropshipper') : __('Billing') }} {{ __('Details') }}
                        </h4>
                    </div>
                    <div class="table-responsive-sm">
                        <table class="table">
                            <tbody>
                                ...
                                <!-- @if ($order->is_dropship === '1')
                                <tr>
                                    <th width="45%">{{ __('Fee Dropshipper') }}</th>
                                    <th width="10%">:</th>
                                    <td width="45%">
                                        @if ($order->is_dropship === '1')
                                            {{ $order->currency_sign }} @rp($feeDropshipper)
                                        @else
                                        0
                                        @endif
                                    </td>
                    ...
                    <div class="table-responsive-sm">
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    <th width="45%">{{ __('Fee Vendor') }}</th>
                                    <th width="10%">:</th>
                                    <td width="45%">{{ $order->currency_sign }} @rp($feeSeller)</td>
                                </tr>
            ...
            @if(count($dropshippers) >= 1)
                @foreach($dropshippers as $dropshipper)
                <div class="col-lg-6">
                    <div class="special-box">
                        <div class="heading-area">
                            <h4 class="title">
                                {{ __('Dropshipper Details') }}
                            </h4>
                        </div>
                            ...
                            <table class="table">
                                <tbody>
                                    <tr>
                                        <th width="45%">{{ __('Shop Name') }}</th>
                                        <th width="10%">:</th>
                                        <td width="45%">{{$dropshipper->shop_name}}</td>
                                    </tr>
                                    <tr>
                                        <th width="45%">{{ __('Email') }}</th>
                                        <th width="10%">:</th>
                                        <td width="45%">{{ $dropshipper->email }}</td>
                                    </tr>                                
                                    <tr>
                                        <th width="45%">{{ __('Phone') }}</th>
                                        <th width="10%">:</th>
                                        <td width="45%">{{ $dropshipper->phone }}</td>
                                    </tr>
                                    <tr>
                                        <th width="45%">{{ __('Fee Dropshipper') }}</th>
                                        <th width="10%">:</th>
                                        <td width="45%">{{ $order->currency_sign }} @rp($feeDropshipper)</td>
                                    </tr>
                                    <tr>
                                        <th width="45%">Rekening Name</th>
                                        <td width="10%">:</td>
                                        <td width="45%">{{ $dropshipper->rekening_name}}</td>

                                    </tr>
                                    <tr>
                                        <th width="45%">No Rekening</th>
                                        <td width="10%">:</td>
                                        <td width="45%">{{$dropshipper->rekening_no}}</td>
                                    </tr>
                                    <tr>
                                        <th width="45%">Bank</th>
                                        <td width="10%">:</td>
                                        <td width="45%">{{$dropshipper->bank}}</td>
                                    </tr>
                                    <tr>
                                        <th width="45%">Cabang</th>
                                        <td width="10%">:</td>
                                        <td width="45%">{{$dropshipper->cabang}}</td>
                                    </tr>                                                                                                                                       
                                </tbody>
                            </table>
                        </div>
            ...
            @foreach($cart->items as $key => $product)
                @php 
                    $dropship[$key] = $product['is_dropship'];
                @endphp
            @endforeach
            @if (in_array(true, $dropship))
            <div class="col-lg-6">
                <div class="special-box">
                    <div class="heading-area">
                        <h4 class="title">
                        ...
                        <table id="example2" class="table table-hover dt-responsive"  cellspacing="0" width="100%">
                            ...
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td>{{$order->currency_sign}} @rp(round( ($product['item']['price'] * $product['qty']) *
